...
public class AddChild extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("Add Mother Vaccine");
...
/*
    ...
    private void startposting() {
        progressDialog.setMessage("Adding mother's vaccine details..");
        progressDialog.show();
        ...
        if(!TextUtils.isEmpty(Cname) && !TextUtils.isEmpty(Cb) && !TextUtils.isEmpty(CFatherName)){
            // Start uploading..
            blog blog1 = new blog("MotherName","DateofVaccine",
                    "VaccineName","UserId");

            DatabaseReference newPost = mPostDatabase.push();

            Map<String,String> DataToSave = new HashMap<>();
            DataToSave.put("MotherName",Cname);
            DataToSave.put("DateofVaccine",Cb);
            DataToSave.put("VaccineName",CFatherName);
            DataToSave.put("UserId",mUser.getUid());

            newPost.setValue(DataToSave);
            Toast.makeText(getApplicationContext(),"Mother's Vaccine added!",Toast.LENGTH_LONG).show();
            mChildName.setText("");
            mChildDOB.setText("");
